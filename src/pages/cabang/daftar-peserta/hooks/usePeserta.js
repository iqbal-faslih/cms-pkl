...
export const usePeserta = (options = {}) => {
  ...
  const queryString = useMemo(() => {
    return buildQuery({
      ...
      filters: {
        status_magang: appliedFilters.map((f) => (f === "aktif" ? "aktif" : "alumni")),
      },
      date: dateFilter,
    });
  }, [
  ...
  const {
    ...
    error,
    refetch,
  } = useFetch(`/cabang-peserta-magang?${queryString}`);
  ...
  const transformedData = useMemo(() => {
    ...
    return apiData.data.map((item, index) => ({
      ...
      originalId: item.id,
      nama: item.name || "N/A",
      tanggalAkhir: item.tanggal_berakhir_magang || "N/A",
      status: item.status_magang,
      sekolah: item.sekolah || "N/A",
      devisi: item.divisi || "Tidak memiliki divisi",
    }));
